...
Session.prototype.givePiece = function(player) {
...
Session.prototype.getNumPlayers = function() {
    var num = 0;
...
Session.prototype.removePlayer = function(playerId) {
    ...
    if (this.getNumPlayers() == 0) {
        console.log('Wiping board ' + this.getNumPlayers());
        this.newGame();
    }
}
...
Session.prototype.clearLines = function() {
    var linesToRemove = [];
    var linesRemoved = 0;
...
Session.prototype.checkPlayerCollision = function(player) {
...
Session.prototype.update = function() {
    for (var key in this.players) {
        ...
        if (player.input['space'].length > 0) {
        ...
        while (piece.progress >= 1) {
            ...
            var collides = piece.collidesWithBoard(this.board);
            if (collides) {
                piece.y -= 1;
                piece.progress = 0;
    ...
    for (var id in this.players) {
        var player = this.players[id];
        var temp = player.piece.color;
        player.piece.color = 0xffffff;
        player.socket.emit('board', {
            'score': this.score,
            'board': this.board.getRendered(pieces)
        });
        player.piece.color = temp;
    }
};
